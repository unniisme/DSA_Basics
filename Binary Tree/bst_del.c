...
int Insert(int item)
{
    ...
    //If tree is empty, set the node as the only node
    if (ROOT == NULL)
    {
        ROOT = add;
        return 0;
...
{
    //If there are no subtrees, simply change the node to NULL and disconnect it from its parents
    ...
    //If both the subtrees exist, replace the node with its predecessor and delete the predecessor (recursively).
    if(target->left != NULL && target->right != NULL)
    {
        node* replacement = Predecessor(target);
        target->item = replacement->item;
        Delete(replacement);
        return 0;
    }
    
    //If only one of the subtrees exist, remove it and reconnect the parent to that subtree in the corresponding place
    if(target->right != NULL)
    {
